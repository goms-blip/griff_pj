...
// 질문 번역용 Gemini (Google AI Studio 키). 없으면 아래 OpenAI 폴백으로 넘어간다.
const GEMINI_API_KEY = (process.env.GEMINI_API_KEY || '').trim();
const GEMINI_MODEL = (process.env.GEMINI_MODEL || 'gemini-3.6-flash').trim();
// 번역 폴백용 OpenAI(GPT). Gemini 가 키 오류·한도 초과·모델 없음 등으로 실패하면 이쪽으로 넘어간다.
//  둘 다 없으면 번역 라우트만 503 으로 막히고 나머지 기능은 정상 동작한다.
...
const mapQuestionRow = (row) => row ? ({
  ...
  // 다국어 번역 캐시 (add_multilang_translation.sql). 미적용이면 빈 객체.
  //  { ko: { title, content, at, by }, en: {...}, zh: {...}, ja: {...} }
  translations: (row.translations && typeof row.translations === 'object') ? row.translations : {},
  // 원문 감지 언어. 신규 컬럼이 없으면 구 translated_lang 으로 폴백.
  source_lang: row.source_lang || row.translated_lang || '',
  created_at: row.created_at,
}) : null;
...
// 🔧 AUDIT §2-1 — Q&A 통합 세션에서 미러 세션 강연자가 403 을 받던 문제.
// ------------------------------------------------------------
//  통합된 질문은 **원본 세션 밑에** 저장된다(qaSessionId). 그래서 질문 기준으로
//  권한을 보면 항상 "원본의 admin_token" 을 요구하게 되고, 미러 세션 링크를 받은
//  강연자는 목록은 보이는데 답변완료·숨김·삭제·번역을 누르는 순간 403 이 났다.
//  (화면은 멀쩡하고 버튼을 눌러야 실패해서 라이브 중에 발견되는 유형)
//
//  이 함수는 **원본 토큰과 그 원본을 바라보는 모든 미러 토큰을 모두 인정**한다.
//  두 룸이 같은 Q&A 를 공유한다는 것은 곧 "양쪽 진행자가 같은 질문을 다룬다" 는 뜻이므로
//  권한을 같이 주는 것이 설계 의도와 맞는다.
async function requireQuestionAdmin(req, res, questionSessionId) {
  const token = extractToken(req);
  if (!token) {
    res.status(401).json({ success: false, message: '토큰이 필요합니다.' });
    return false;
  ...
  // 질문이 달린 세션(= 통합 시 원본) + 그 세션을 원본으로 쓰는 미러 세션들
  const { data: own, error: e1 } = await supabase
    .from('sessions').select('id, admin_token').eq('id', questionSessionId).maybeSingle();
  if (e1) {
    res.status(500).json({ success: false, message: 'DB 조회 중 오류가 발생했습니다.' });
    return false;
  }
  ...
  if (own.admin_token === token) return true;

  // qa_parent_id 컬럼이 없는 환경(add_session_qa_merge.sql 미적용)이면 미러가 있을 수 없다.
  const { data: mirrors, error: e2 } = await supabase
    .from('sessions').select('admin_token').eq('qa_parent_id', questionSessionId);
  if (e2 && !isSchemaMissing(e2)) {
    res.status(500).json({ success: false, message: 'DB 조회 중 오류가 발생했습니다.' });
    return false;
  }
  if ((mirrors || []).some((m) => m.admin_token === token)) return true;
...
app.post('/api/admin/questions/:id/answered', wrap(async (req, res) => {
  ...
  const ok = await requireQuestionAdmin(req, res, sessionId);
  if (!ok) return;
...
// ============================================================
// 🌐 질문 번역 (Gemini → GPT 폴백) — 한국어·영어·중국어·일본어
// ------------------------------------------------------------
//  운영자/연사가 [한][영][중][일] 버튼을 누르면 해당 언어로 옮기고
//  결과를 questions.translations 에 언어별로 캐시한다(add_multilang_translation.sql).
//   translations = { ko: {title, content, at, by}, en: {...}, zh: {...}, ja: {...} }
//
//  제공자(provider) 순서:
//   1) Gemini  — GEMINI_MODEL → GEMINI_FALLBACK_MODELS (모델 없음 404 시 다음 후보)
//   2) OpenAI  — OPENAI_MODEL → OPENAI_FALLBACK_MODELS (Gemini 가 어떤 이유로든 실패했을 때)
//  Gemini 키가 아예 없으면 곧바로 OpenAI 로 간다. 둘 다 없으면 503.
//
//  컬럼이 없으면 저장만 건너뛰고 번역 결과는 그대로 응답한다(하위호환).
// ============================================================
const TRANSLATE_SCHEMA_MSG = '번역 결과를 저장하려면 add_multilang_translation.sql 을 먼저 실행해 주세요. (이번 번역은 저장되지 않았습니다)';

// 지원 대상 언어. 프론트의 언어 버튼과 1:1 로 대응한다.
//  name  — 프롬프트에 넣을 언어 이름
//  label — 안내 메시지에 쓸 한글 라벨
const TRANSLATE_TARGETS = {
  ko: { name: '한국어 (Korean)',             label: '한국어' },
  en: { name: '영어 (English)',              label: '영어' },
  zh: { name: '중국어 간체 (Simplified Chinese)', label: '중국어' },
  ja: { name: '일본어 (Japanese)',           label: '일본어' },
};
const DEFAULT_TARGET = 'ko';

// 요청 body 의 target 을 정규화. 모르는 값이면 한국어로.
const normalizeTarget = (v) => {
  const t = (v || '').toString().trim().toLowerCase();
  if (TRANSLATE_TARGETS[t]) return t;
  // zh-cn / zh-tw / kr / jp 같은 흔한 변형을 흡수한다.
  if (t.startsWith('zh')) return 'zh';
  if (t.startsWith('ja') || t === 'jp') return 'ja';
  if (t.startsWith('ko') || t === 'kr') return 'ko';
  if (t.startsWith('en')) return 'en';
  return DEFAULT_TARGET;
};

// 대상 언어별 시스템 프롬프트. 규칙은 언어와 무관하게 동일하고 목표 언어만 갈아끼운다.
const buildTranslatePrompt = (target) => {
  const t = TRANSLATE_TARGETS[target] || TRANSLATE_TARGETS[DEFAULT_TARGET];
  return [
    `너는 행사 Q&A 운영 콘솔의 번역기다. 참가자가 남긴 질문을 진행자와 연사가 읽을 수 있도록 ${t.name} 로 옮긴다.`,
    '',
    '입력은 {"title": "...", "content": "..."} 형태의 JSON 이다.',
    '출력도 반드시 {"source_lang": "...", "title": "...", "content": "..."} 형태의 JSON 하나만 낸다. 규칙:',
    '- source_lang 에는 **원문**의 언어를 ISO 639-1 코드로 넣는다 (ko, en, ja, zh, vi, th, ...). 판별이 어려우면 "und".',
    `- title/content 에는 ${t.name} 로 옮긴 문장을 넣는다.`,
    `- 원문이 이미 ${t.name} 면 번역하지 말고 원문을 그대로 title/content 에 넣는다.`,
...
//  ⚠️ 2026-08 확인: 현재 GEMINI_API_KEY 로 gemini-2.5-flash 는 404
//     ("no longer available to new users") 라서 gemini-flash-latest 를 앞에 둔다.
const GEMINI_FALLBACK_MODELS = ['gemini-flash-latest', 'gemini-2.5-flash'];
const OPENAI_FALLBACK_MODELS = ['gpt-4o-mini', 'gpt-4o'];

// 모델이 응답한 JSON 을 우리 형태로 정규화. 두 제공자가 공용으로 쓴다.
const normalizeTranslation = (out) => ({
...
async function geminiTranslate(title, content, target) {
  if (!GEMINI_API_KEY) throw publicErr(503, 'GEMINI_API_KEY 가 없습니다.');
  const chain = [GEMINI_MODEL, ...GEMINI_FALLBACK_MODELS.filter((m) => m !== GEMINI_MODEL)];
  let lastErr;
  for (const model of chain) {
    try {
      return await geminiTranslateWith(model, title, content, target);
    } catch (e) {
      lastErr = e;
      // 모델 없음(404) 일 때만 다음 후보로 넘어간다. 키 오류·한도 초과 등은 즉시 중단(→ GPT 폴백).
...
async function geminiTranslateWith(model, title, content, target) {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`;
  const payload = {
    systemInstruction: { parts: [{ text: buildTranslatePrompt(target) }] },
  ...
  if (!res.ok) {
    ...
    if (res.status === 404) {
      ...
      const e404 = publicErr(502, `Gemini 모델 '${model}' 을 찾을 수 없습니다. GEMINI_MODEL 환경변수를 확인해 주세요.`);
      e404.geminiModelMissing = true;
      throw e404;
    }
    if (res.status === 429) throw publicErr(429, 'Gemini 번역 요청이 한도를 초과했습니다.');
  ...
  return normalizeTranslation(out);
}

// ------------------------------------------------------------
// 2차(예비) 제공자: OpenAI GPT
// ------------------------------------------------------------
//  Gemini 가 실패했을 때만 호출된다. Chat Completions + JSON 모드를 쓴다.
async function openaiTranslate(title, content, target) {
  if (!OPENAI_API_KEY) throw publicErr(503, 'OPENAI_API_KEY 가 없습니다.');
  const chain = [OPENAI_MODEL, ...OPENAI_FALLBACK_MODELS.filter((m) => m !== OPENAI_MODEL)];
  let lastErr;
  for (const model of chain) {
    try {
      return await openaiTranslateWith(model, title, content, target);
    } catch (e) {
      lastErr = e;
      if (e && e.openaiModelMissing) {
        console.warn(`[openai] 모델 '${model}' 사용 불가 → 다음 후보로 재시도`);
        continue;
      }
      throw e;
    }
...
async function openaiTranslateWith(model, title, content, target) {
  const payload = {
    model,
    messages: [
      { role: 'system', content: buildTranslatePrompt(target) },
      { role: 'user', content: JSON.stringify({ title: title || '', content: content || '' }) },
    ],
    response_format: { type: 'json_object' },
  };
  // gpt-5 / o 시리즈는 temperature 커스텀을 받지 않는다 → 해당 모델에는 아예 안 보낸다.
  if (!/^(gpt-5|o\d)/i.test(model)) payload.temperature = 0.2;
  ...
  try {
    ...
    if (e && e.name === 'TimeoutError') throw publicErr(504, '번역 응답이 지연되고 있습니다. 잠시 후 다시 시도해 주세요.');
    throw publicErr(502, '예비 번역 서버(OpenAI)에 연결하지 못했습니다.');
  }

  const raw = await res.text();
  if (!res.ok) {
    console.error('[openai] translate failed:', res.status, raw.slice(0, 500));
    if (res.status === 401 || res.status === 403) {
      throw publicErr(502, 'OpenAI API 키가 올바르지 않거나 권한이 없습니다. OPENAI_API_KEY 를 확인해 주세요.');
    }
    // 모델이 없거나 계정에서 안 열린 경우 → 다음 후보 모델로.
    if (res.status === 404 || (res.status === 400 && /model/i.test(raw))) {
      const eModel = publicErr(502, `OpenAI 모델 '${model}' 을 사용할 수 없습니다. OPENAI_MODEL 환경변수를 확인해 주세요.`);
      eModel.openaiModelMissing = true;
      throw eModel;
    }
    if (res.status === 429) throw publicErr(429, '번역 요청이 한도를 초과했습니다. 잠시 후 다시 시도해 주세요.');
    throw publicErr(502, `번역에 실패했습니다. (OpenAI ${res.status})`);
  }

  let parsed;
  try { parsed = JSON.parse(raw); } catch (e) { throw publicErr(502, '번역 응답을 해석하지 못했습니다.'); }
  const msg = parsed && parsed.choices && parsed.choices[0] && parsed.choices[0].message;
  const text = (msg && msg.content ? msg.content : '').toString().trim();
  if (!text) {
    const reason = (parsed.choices && parsed.choices[0] && parsed.choices[0].finish_reason) || '';
    console.error('[openai] empty message:', reason, raw.slice(0, 500));
    throw publicErr(502, '번역 결과가 비어 있습니다. 다시 시도해 주세요.');
  }
  let out;
  try { out = JSON.parse(text); } catch (e) {
    console.error('[openai] non-JSON output:', text.slice(0, 500));
    throw publicErr(502, '번역 결과 형식이 올바르지 않습니다. 다시 시도해 주세요.');
  }
  return normalizeTranslation(out);
}
...
// ------------------------------------------------------------
// 제공자 오케스트레이션 — Gemini 먼저, 실패하면 GPT
// ------------------------------------------------------------
//  반환: { source_lang, title, content, provider: 'gemini' | 'openai' }
async function translateText(title, content, target) {
  if (!GEMINI_API_KEY && !OPENAI_API_KEY) {
    throw publicErr(503, '번역 기능이 설정되지 않았습니다. 서버 환경변수 GEMINI_API_KEY 또는 OPENAI_API_KEY 를 등록해 주세요.');
  }

  let geminiErr = null;
  if (GEMINI_API_KEY) {
    try {
      const out = await geminiTranslate(title, content, target);
      return { ...out, provider: 'gemini' };
    } catch (e) {
      geminiErr = e;
      // 폴백 키가 없으면 여기서 끝 — Gemini 의 원래 오류를 그대로 올린다.
      if (!OPENAI_API_KEY) throw e;
      console.warn('[translate] Gemini 실패 → OpenAI(GPT) 폴백:', (e && e.message) || e);
    }
  }

  try {
    const out = await openaiTranslate(title, content, target);
    return { ...out, provider: 'openai' };
  } catch (e) {
    // 둘 다 실패 — 원인 파악이 쉽도록 두 오류를 함께 알린다.
    if (geminiErr) {
      throw publicErr(
        e.status || 502,
        `번역에 실패했습니다. Gemini: ${geminiErr.publicMessage || geminiErr.message} / GPT: ${e.publicMessage || e.message}`
      );
    }
    throw e;
  }
...
// POST /api/admin/questions/:id/translate — body { target?: 'ko'|'en'|'zh'|'ja', force?: boolean } [세션admin]
//   target 생략 시 한국어. force=true 면 캐시를 무시하고 다시 번역한다.
app.post('/api/admin/questions/:id/translate', wrap(async (req, res) => {
  ...
  const target = normalizeTarget(req.body && req.body.target);
  const force = !!(req.body && req.body.force);

  // 이미 그 언어로 번역돼 있으면 API 호출 없이 캐시 반환 (force 면 무시)
  const cache = (q.translations && typeof q.translations === 'object') ? q.translations : {};
  const hit = cache[target];
  if (!force && hit && hit.content) {
    return res.json({ success: true, data: mapQuestionRow(q), cached: true, target });
  }
  // 다국어 컬럼이 아직 없는 환경의 하위호환 — 한국어는 구 컬럼 캐시도 인정한다.
  if (!force && target === 'ko' && !q.translations && q.translated_content) {
    return res.json({ success: true, data: mapQuestionRow(q), cached: true, target });
  }

  const t = await translateText(q.title, q.content, target);
  const nowIso = new Date().toISOString();

  const nextTranslations = { ...cache };
  nextTranslations[target] = { title: t.title, content: t.content, at: nowIso, by: t.provider };

  // 신규 컬럼(다국어) + 구 컬럼(한국어 하위호환)을 함께 채운다.
  const legacyPatch = target === 'ko' ? {
    translated_title: t.title,
    translated_content: t.content,
    translated_lang: t.source_lang,
    translated_at: nowIso,
  } : {};
  const fullPatch = { translations: nextTranslations, source_lang: t.source_lang, ...legacyPatch };

  let { data: updated, error: uErr } = await supabase
    .from('questions').update(fullPatch).eq('id', req.params.id).select().maybeSingle();

  if (uErr && isSchemaMissing(uErr)) {
    // add_multilang_translation.sql 미적용 → 구 컬럼에만이라도 저장을 시도한다(한국어일 때).
    if (target === 'ko') {
      ({ data: updated, error: uErr } = await supabase
        .from('questions').update(legacyPatch).eq('id', req.params.id).select().maybeSingle());
    } else {
      uErr = { code: '42703' }; // 저장할 곳이 없음 → 아래 미저장 응답으로
    }
  }

  if (uErr) {
    if (isSchemaMissing(uErr)) {
      // 저장은 포기하고 번역 결과만 돌려준다(화면에는 정상 표시).
      return res.json({
        success: true,
        data: mapQuestionRow({ ...q, ...fullPatch }),
        cached: false, saved: false, target, provider: t.provider, message: TRANSLATE_SCHEMA_MSG,
      });
    }
    throw uErr;
  }
  if (!updated) return res.status(404).json({ success: false, message: '질문을 찾을 수 없습니다.' });
  // 구 컬럼에만 저장된 환경에서도 화면이 다국어 캐시를 볼 수 있도록 메모리상 병합해서 응답.
  res.json({
    success: true,
    data: mapQuestionRow({ ...updated, translations: nextTranslations, source_lang: t.source_lang }),
    cached: false, saved: true, target, provider: t.provider,
  });
}));

// ============================================================
// 🛣️ 라우트: 세션 종료 설문조사 (고정 3문항)
// ------------------------------------------------------------
//  Q1 세션 만족도(1~5) / Q2 연사 전달력·추천 의향(1~5) / Q3 자유 의견(주관식)
//
//  노출 트리거(프론트가 판단, 둘 중 하나만 만족해도 뜬다):
//   1) 자동 — 세션 ends_at 이 지나면 참가자 브라우저가 스스로 띄운다.
//   2) 수동 — 운영자가 대시보드에서 [설문 열기] → sessions.survey_open = true.
//
//  ⚠️ 응답 쓰기는 이 서버(service_role)만 한다. anon RLS 정책은 열지 않는다.
//     1인 1응답(respondent_key)·점수 범위·길이 검증을 서버에서 강제하기 위함.
//  ⚠️ Q&A 통합(qa_parent_id) 세션이라도 설문은 **참가자가 실제로 본 세션**에 남긴다.
//     질문은 공유해도 만족도/연사 평가는 세션마다 따로 봐야 하기 때문.
// ============================================================
const SURVEY_SCHEMA_MSG = '설문 기능을 사용하려면 add_survey.sql 을 먼저 실행해 주세요.';

// 1~5 정수로 정규화. 값이 없거나 범위를 벗어나면 null(=미응답).
const surveyScore = (v) => {
...
const SURVEY_LANGS = ['ko', 'en', 'zh', 'ja'];
const surveyLang = (v) => {
  const s = (v || '').toString().trim().toLowerCase().slice(0, 5);
  return SURVEY_LANGS.includes(s) ? s : 'ko';
};

// 응답 배열 → 집계. 프론트가 그대로 그릴 수 있는 형태로 만든다.
const summarizeSurvey = (rows) => {
  const list = rows || [];
  const avg = (key) => {
  ...
  const dist = (key) => {
    ...
    list.forEach((r) => { if (d[r[key]] !== undefined) d[r[key]] += 1; });
    return d;
  };
  return {
    count: list.length,
    q1_avg: avg('q1_score'),
    q2_avg: avg('q2_score'),
    q1_dist: dist('q1_score'),
    q2_dist: dist('q2_score'),
    comment_count: list.filter((r) => (r.q3_text || '').trim()).length,
  };
};

// POST /api/admin/sessions/:sessionId/survey/open — body { open: boolean } [세션admin]
//   운영자가 Q&A 를 끝내고 설문을 즉시 띄우고 싶을 때. 끄면 자동(종료시각) 트리거만 남는다.
app.post('/api/admin/sessions/:sessionId/survey/open', wrap(async (req, res) => {
  const session = await resolveSession(req.params.sessionId);
  if (!session) return res.status(404).json({ success: false, message: '세션을 찾을 수 없습니다.' });
  const ok = await requireSessionAdmin(req, res, session.id);
  if (!ok) return;
  ...
  const patch = { survey_open: open, survey_opened_at: open ? new Date().toISOString() : null };

  const { data, error } = await supabase
    .from('sessions').update(patch).eq('id', session.id).select().maybeSingle();
  if (error) {
    if (isSchemaMissing(error)) return res.status(400).json({ success: false, message: SURVEY_SCHEMA_MSG });
    throw error;
  }
  if (!data) return res.status(404).json({ success: false, message: '세션을 찾을 수 없습니다.' });
  res.json({
    success: true,
    data: { survey_open: !!data.survey_open, survey_opened_at: data.survey_opened_at || null },
  });
}));

// GET /api/admin/sessions/:sessionId/survey — 설문 상태 + 집계 + 응답 목록 [세션admin]
app.get('/api/admin/sessions/:sessionId/survey', wrap(async (req, res) => {
  const session = await resolveSession(req.params.sessionId);
  if (!session) return res.status(404).json({ success: false, message: '세션을 찾을 수 없습니다.' });
  const ok = await requireSessionAdmin(req, res, session.id);
  if (!ok) return;
  ...
  const { data, error } = await supabase
    ...
    .select('id, q1_score, q2_score, q3_text, lang, created_at')
    .eq('session_id', session.id)
    .order('created_at', { ascending: false });

  if (error) {
    // add_survey.sql 미적용 → 빈 집계로 안내만 내린다(대시보드가 죽지 않도록).
    if (isSchemaMissing(error)) {
      return res.json({
        success: true,
        data: {
          survey_open: false, survey_opened_at: null, ends_at: session.ends_at || null,
          summary: summarizeSurvey([]), responses: [], schema_ready: false, message: SURVEY_SCHEMA_MSG,
        },
      });
    }
    throw error;
  ...
  res.json({
    success: true,
    data: {
      survey_open: !!session.survey_open,
      survey_opened_at: session.survey_opened_at || null,
      ends_at: session.ends_at || null,
      summary: summarizeSurvey(data || []),
      responses: data || [],
      schema_ready: true,
    },
  });
...
// POST /api/public/sessions/:codeOrId/survey — 참가자 설문 응답 제출 (무인증)
//   body { respondent_key, q1_score?, q2_score?, q3_text?, lang? }
//   같은 respondent_key 로 다시 내면 덮어쓴다(응답 수정 허용).
app.post('/api/public/sessions/:codeOrId/survey', wrap(async (req, res) => {
  ...
  // 세션 조회와 동일하게 비공개 세션은 존재 여부까지 숨긴다.
  if (!canView) return res.status(404).json({ success: false, message: '세션을 찾을 수 없습니다.' });

  const b = req.body || {};
  const respondentKey = (b.respondent_key || '').toString().trim().slice(0, 100);
  if (!respondentKey) {
    return res.status(400).json({ success: false, message: '응답자 식별값이 없습니다. 페이지를 새로고침해 주세요.' });
  }

  const q1 = surveyScore(b.q1_score);
  const q2 = surveyScore(b.q2_score);
  const q3 = (b.q3_text || '').toString().trim().slice(0, 500);
  // 3문항 전부 비어 있으면 저장할 게 없다.
  if (q1 === null && q2 === null && !q3) {
    return res.status(400).json({ success: false, message: '한 문항 이상 응답해 주세요.' });
  }
  ...
  const row = {
    ...
    q2_score: q2,
    q3_text: q3 || null,
    lang: surveyLang(b.lang),
    updated_at: new Date().toISOString(),
  };

  const { data, error } = await supabase
    .from('survey_responses')
    .upsert(row, { onConflict: 'session_id,respondent_key' })
    .select('id, created_at')
    .maybeSingle();

  if (error) {
    if (isSchemaMissing(error)) {
      return res.status(503).json({ success: false, message: SURVEY_SCHEMA_MSG });
    }
    throw error;
  }
  res.json({ success: true, data: { id: data && data.id, created_at: data && data.created_at } });
}));
...
app.get('/api/public/sessions/:codeOrId', wrap(async (req, res) => {
  ...
  res.json({
    ...
    data: {
      ...
      // 📝 설문 노출 판단용 (add_survey.sql). 컬럼이 없으면 false/null.
      //  프론트는 survey_open 이거나 ends_at 이 지났으면 설문 모달을 띄운다.
      //  ⚠️ 설문은 통합 원본이 아니라 **이 세션**에 남긴다(연사 평가는 세션별).
      survey_open: !!session.survey_open,
      survey_opened_at: session.survey_opened_at || null,
    },
  });
...
// ------------------------------------------------------------
//  🔧 AUDIT §1-3 · §1-6 (P0-5) — 참가자 화면의 **주 데이터 경로**로 승격했다.
//   예전에는 참가자 브라우저가 anon 으로 Supabase 를 직접 읽었다. 그러면
//   1,800~6,000명이 20초마다 목록 전체를 받아 **Supabase 이그레스**를 그대로 태운다
//   (행사 2일에 150~500GB 추정). 성능이 아니라 쿼터·비용 문제라 부하 테스트로는 안 잡힌다.
//
//   이 라우트를 거치면 Vercel 엣지가 s-maxage 동안 **원본 요청 1건으로 흡수**한다.
//   와이파이가 출렁여 전원이 동시에 재동기화해도(§1-6) 엣지에서 끝난다.
//
//  ⚠️ 캐시 조건: **공개 세션 + 미리보기 토큰 없음** 일 때만. 토큰이 붙은 요청은
//     비공개 세션 미리보기라 절대 공유 캐시에 올리면 안 된다(no-store).
//  ⚠️ ?limit= 은 응답 크기 상한(AUDIT §1-2c). URL 이 다르면 캐시 키도 달라진다.
const PUBLIC_Q_MAX_LIMIT = 200;      // 방어 상한 — 이보다 크게 요청해도 잘린다
const PUBLIC_Q_CACHE_SECONDS = 5;    // 엣지 보관 시간. 참가자 체감 지연은 최대 5초
app.get('/api/public/sessions/:codeOrId/questions', wrap(async (req, res) => {
  ...
  // 상한: 미지정이면 전체(기존 동작). 숫자가 오면 1~200 으로 죈다.
  const rawLimit = parseInt((req.query.limit || '').toString(), 10);
  const limit = isNaN(rawLimit) ? null : Math.max(1, Math.min(PUBLIC_Q_MAX_LIMIT, rawLimit));
  ...
  // 🧊 엣지 캐시 — 공개 세션이고 토큰이 없을 때만.
  //   stale-while-revalidate 로 만료 직후 요청도 대기 없이 옛 값을 받고 뒤에서 갱신된다.
  if (session.is_public === true && !previewToken) {
    res.setHeader('Cache-Control',
      `public, s-maxage=${PUBLIC_Q_CACHE_SECONDS}, stale-while-revalidate=25`);
  } else {
    res.setHeader('Cache-Control', 'no-store');
  }
  ...
  res.json({
    success: true,
    // total 은 상한에 걸려 잘렸는지 판단용(프론트가 "전체 N개 중" 표기에 사용)
...
app.get('/api/public/projects/:projectId/landing', wrap(async (req, res) => {
  ...
  // 스키마 적용 정도에 따라 위에서부터 시도한다(없는 컬럼이면 42703 → 다음 후보).
  //  qa_parent_id 는 질문 수 집계에 필요하다(AUDIT §5-1) — 없으면 통합 전 동작 그대로.
  const candidates = [
    `id, code, ${baseCols.replace('id, ', '')}, track_id, speaker, qa_parent_id`,
  ...
  const list = sessions || [];
  // 🔧 AUDIT §5-1 — Q&A 통합 세션의 랜딩 질문 수가 0 으로 보이던 문제.
  //   통합 세션의 질문은 **원본 세션 밑에** 쌓이므로, 자기 id 로 세면 항상 0 이다.
  //   집계에는 "질문이 실제로 달린 id"(qaSessionId)를 쓰고, 표시할 때 되돌린다.
  const qaIdOf = (s) => s.qa_parent_id || s.id;
  const sessionIds = Array.from(new Set(list.map(qaIdOf)));
  ...
  const result = {
    ...
    sessions: list.map((row) => ({
      ...
      // 통합 세션이면 원본 id 로 센 값을 보여준다(두 룸이 같은 수를 본다)
      questionCount: countMap[qaIdOf(row)] || 0,
    })),
  };
...
const EXCEL_COLUMNS = [
  ...
  // 다국어 번역 캐시 (add_multilang_translation.sql). 번역 안 한 언어는 빈 칸.
  { header: 'source_lang', key: 'source_lang', width: 12 },
  { header: 'ko', key: 'tr_ko', width: 40 },
  { header: 'en', key: 'tr_en', width: 40 },
  { header: 'zh', key: 'tr_zh', width: 40 },
  { header: 'ja', key: 'tr_ja', width: 40 },
...
const SURVEY_COLUMNS = [
  { header: 'session_title', key: 'session_title', width: 24 },
  { header: 'response_id', key: 'response_id', width: 38 },
  { header: 'q1_세션만족도', key: 'q1_score', width: 14 },
  { header: 'q2_연사평가', key: 'q2_score', width: 14 },
  { header: 'q3_자유의견', key: 'q3_text', width: 60 },
...
// translations jsonb 에서 한 언어를 "제목 / 본문" 한 칸으로 펴서 꺼낸다.
//  구 스키마(translated_*)만 있는 환경에서는 한국어를 그쪽에서 가져온다.
const translationCell = (q, lang) => {
  const t = (q.translations && typeof q.translations === 'object') ? q.translations[lang] : null;
  if (t && (t.title || t.content)) return `${t.title || ''}\n${t.content || ''}`.trim();
  if (lang === 'ko' && q.translated_content) {
    return `${q.translated_title || ''}\n${q.translated_content}`.trim();
  }
  return '';
};

// rows: 질문 배열 / surveyRows: 설문 응답 배열(없으면 설문 시트를 만들지 않음)
const buildWorkbook = async (rows, projectTitle, sessionTitleMap, surveyRows) => {
  ...
  rows.forEach((q) => {
    ws.addRow({
      ...
      source_lang: q.source_lang || q.translated_lang || '',
      tr_ko: translationCell(q, 'ko'),
      tr_en: translationCell(q, 'en'),
      tr_zh: translationCell(q, 'zh'),
      tr_ja: translationCell(q, 'ja'),
      exported_at: exportedAt,
    });
  });

  // 📝 설문 응답 시트 — 응답이 하나라도 있을 때만 추가한다.
  if (surveyRows && surveyRows.length) {
    const sws = wb.addWorksheet('설문');
    sws.columns = SURVEY_COLUMNS;
    sws.getRow(1).font = { bold: true };
    surveyRows.forEach((r) => {
      sws.addRow({
        session_title: sessionTitleMap[r.session_id] || '',
        response_id: r.id,
        q1_score: r.q1_score,
        q2_score: r.q2_score,
        q3_text: r.q3_text || '',
...
// 세션 id 목록의 설문 응답을 가져온다. add_survey.sql 미적용이면 [] (엑셀에서 시트만 빠짐).
const fetchSurveyRows = async (sessionIds) => {
  const ids = (sessionIds || []).filter(Boolean);
  if (!ids.length) return [];
  const { data, error } = await supabase
    .from('survey_responses')
    .select('id, session_id, q1_score, q2_score, q3_text, lang, created_at')
    .in('session_id', ids)
    .order('created_at', { ascending: true });
  if (error) {
    if (isSchemaMissing(error)) return [];
...
app.get('/api/admin/sessions/:sessionId/questions/export', wrap(async (req, res) => {
  ...
  // 설문은 통합 원본이 아니라 이 세션 자신에 쌓인다.
  const surveyRows = await fetchSurveyRows([sessionId]);
  const wb = await buildWorkbook(questions || [], projectTitle, sessionTitleMap, surveyRows);
...
app.get('/api/admin/projects/:projectId/questions/export', requireConsole, wrap(async (req, res) => {
  ...
  const surveyRows = await fetchSurveyRows(sessionIds);
  // 설문 시트도 세션명 오름차순 → 시간순으로 정렬해 둔다.
  surveyRows.sort((a, b) => {
    const ta = sessionTitleMap[a.session_id] || '';
    const tb = sessionTitleMap[b.session_id] || '';
    if (ta !== tb) return ta < tb ? -1 : 1;
  ...
  const wb = await buildWorkbook(questions, project.title, sessionTitleMap, surveyRows);
