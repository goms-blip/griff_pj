...
### 53) 🖨 룸 QR — 'QR 만' SVG 옵션 추가
- **지시:** "QR만 있는 SVG도 뽑을 수 있게 옵션 추가해줘" (52번의 폰트 대체 우려에 대한 후속)
- **처리:** `buildRoomSvg(url, name, { bare })` 로 분기. `bare` 면 라벨 그룹을 빼고 **정사각 70×70mm**(viewBox `0 0 n n`)로 출력 — 텍스트가 없으니 인쇄소 한글 폰트 문제 자체가 사라진다. 모달 하단에 체크박스 하나로 두 형태를 전환(룸별 `SVG` 버튼과 `전체 SVG` 둘 다 반영), 버튼 라벨에도 `· QR 만` 표기.
- **파일명으로 구분:** 라벨 포함 `QR_<룸>.svg` / QR 만 `QR_<룸>_qr-only.svg` — 인쇄소에 섞여 넘어가는 사고 방지.
- **검증(로컬, 다운로드 가로채기):**
  | 옵션 | 파일명 | 크기 | viewBox | `<text>` |
  |---|---|---|---|---|
  | 기본 | `QR_Harmony Ballroom 1.svg` | 70×93.8mm | `0 0 33 44.22` | 있음 |
  | QR 만 | `QR_Harmony Ballroom 1_qr-only.svg` | 70×70mm | `0 0 33 33` | **없음** |
  체크박스 토글 시 즉시 반영, 모달 레이아웃 정상, 콘솔 에러 0.
- **배포:** `dpl_DCxJ8bA9SCFpVsVbNxqZqWGqi3D7` READY.

### 52) 🖨 룸 QR 인쇄용 SVG 다운로드
- **지시:** "각룸 QR은 인쇄용으로도 제작이 가능하도록 SVG파일로 다운로드 받을수 있게도 해줘"
- **가능 여부 먼저 확인:** 브라우저 번들(`qrcode@1.5.1`)에 `toString(text, {type:'svg'})` 이 실제로 있는지 런타임에서 검사 → `create/toCanvas/toDataURL/toString` 모두 존재, SVG 출력 정상. (없었으면 `create()` 의 모듈 행렬로 직접 path 를 그릴 계획이었음)
- **구현(`RoomQrModal`)**
  - `buildRoomSvg(url, roomName)` — 라이브러리 SVG 에서 **QR 도형(path)만 추출**해, 룸 이름 + 안내문을 붙인 새 SVG 로 감싼다.
  - **물리 크기를 mm 로 지정**(QR 폭 70mm → 전체 70×93.8mm). `viewBox` 는 모듈 격자 단위라 QR 버전(격자 크기)이 달라져도 **텍스트 비율이 함께 따라간다.**
  - QR 부분만 `shape-rendering="crispEdges"`, 텍스트는 `auto` 로 되돌려 글자가 뭉개지지 않게 분리.
  - 룸 카드 버튼을 `URL / PNG / SVG` 3개로, 하단에 **전체 PNG / 전체 SVG** 일괄 버튼. "인쇄물엔 SVG, 공유엔 PNG" 안내 문구 추가.
  - 파일명 정리(`safeName`)와 Blob 저장(`saveBlob`)을 헬퍼로 분리, PNG 경로도 같이 사용.
- **검증(로컬):** 실제 다운로드를 가로채 내용 확인 — `image/svg+xml`, 2,073 bytes, `width=70mm height=93.8mm viewBox="0 0 33 44.22"`, `<title>Atlas — LiveQnA 룸 QR</title>`, 흰 배경 + QR path + 룸 이름/안내문. 브라우저에 직접 렌더해 **QR·룸 이름·안내문이 의도대로 보이는 것까지 확인.**
- **⚠️ 남은 확인:** SVG 안의 글자는 **텍스트로** 들어가 있어(패스 변환 아님) 인쇄소 환경에 한글 폰트가 없으면 대체 폰트로 바뀔 수 있다. QR 자체는 벡터 도형이라 무관. 인쇄 전 **실물 1장 스캔 테스트** 권장.
- **배포:** `dpl_93aJPgiEkPT6gD94o4rdeDwKPccM` READY.
...
- **설계 판단:** 세션을 여러 트랙에 거는 다대다 구조(`session_tracks`)가 개념상 깔끔하지만, 랜딩 그룹핑·관리자 카드·엑셀·시트 동기화가 전부 `sessions.track_id` 를 전제로 짜여 있어 **행사 3주 전에 손대기엔 파급이 크다.** → **세션 레코드는 룸마다 그대로 두고 '질문을 읽고 쓰는 대상'만 원본으로 돌리는** 최소 변경을 택함.
  - 핵심: `qaSessionId(session) = qa_parent_id || id`. 프론트는 `resolvedId` 한 곳만 이 값으로 바꾸면 **목록·등록·좋아요·Realtime·폴링이 전부 따라온다.**
  - `questions.session_id` 는 건드리지 않는다 → 통합은 참조 변경일 뿐이라 **언제든 해제 가능**(데이터 이동 없음).
- **구현**
  - `add_session_qa_merge.sql` — `sessions.qa_parent_id`(self FK, `on delete set null`) + 부분 인덱스 + 자기참조 금지 CHECK. **사용자 실행 완료.**
  - `server.js` — `qaSessionId()` 헬퍼. 공개 세션 응답에 `qa_session_id`/`qa_merged` 추가. 반영 지점: 공개 질문 목록·관리자 질문 목록·세션 통계·세션 엑셀 export. PATCH 에 `qa_parent_id` 수용 + 검증 4종. 컬럼 없는 환경 폴백에도 키 추가.
  - `index.html` — 참가자 `resolvedId = meta.qa_session_id || meta.id`. 세션 수정 모달에 **`Q&A 통합` 드롭다운**(수정 모드 + 후보는 자기 제외·이미 미러인 세션 제외). 서버 거절 사유를 토스트로 그대로 노출.
- **검증 (버리는 프로젝트 '룸 하나/룸 둘' → 삭제)**
  - 통합 전 각자 1건/0건 → 연결 후 **양쪽 다 같은 목록**. 미러 화면에서 등록한 질문이 원본 화면에도 표시되고 좋아요도 합산.
  - 운영자: 두 세션 대시보드가 동일 목록, **미러에서 숨김 → 원본에도 반영**. 세션 통계도 양쪽 동일(2건).
  - 검증 로직 4종 전부 정상 거절: 자기 자신 / **체인** / 없는 세션 / 다른 행사.
  - 해제 시 원본 2건·미러 0건 → 참조만 바뀐 것 확인.
  - ⚠️ 첫 브라우저 시도에서 미러가 0건으로 떴는데 **캐시된 구버전 JS** 였다(질문이 미러 세션으로 들어감). 강제 새로고침 후 정상. 흔적 질문은 삭제. → **CDN 캐시(s-maxage=300) 도입 이후 배포 직후 UI 검증은 반드시 하드 리로드**.
- **실제 적용:** NTE 8/21 16:00 — `12a651`(Ballroom 2) → `bf1ba1`(Ballroom 1) 원본으로 통합 완료. 두 코드 모두 `qa_session_id = 4c58c7f8…` 확인.
- **정리:** 검증 프로젝트 삭제, `post_throttle` 비움 → questions 0 / votes 0 / post_throttle 0, sessions 45 · tracks 4 유지.
- **배포:** `dpl_EaX1vzNp3i5Y7GLs1K7m9yFYhgM7` READY.
- **문서:** 개발자 설명서 §6 에 함정 ⑩ 신설(**새 경로에서 `session.id` 를 그대로 질문 조회에 쓰면 통합이 깨진다 — 반드시 `qaSessionId()` 경유**), SQL 순서 11번 추가. 핸드오버의 NTE 경고를 "통합 완료"로 교체하고 행사 중 절차도 갱신.

### 48) 📗 데이터 이슈 2건 종결 — NTE 세션은 '중복'이 아니라 두 룸 병행
- **지시:** "1번은 내가 수정하고 2번은 동일한 세션 맞아"
- **① 8/20 16:00 강연자 공란** — 담당자가 직접 수정하기로. (35번에서 내가 날린 값)
- **② "이환(NTE…)" 8/21 16:00 두 룸** — 물어본 결과 **의도된 병행 진행**(중복 등록 아님). **삭제하지 않는다.**
  - 확인한 실제 상태: 같은 시각·같은 제목의 **별개 세션 레코드 2건** — `bf1ba1`(Harmony Ballroom 1) / `12a651`(Harmony Ballroom 2), 둘 다 공개.
  - **⚠️ 그래서 생기는 실제 영향: Q&A 가 룸별로 갈라진다.** 세션 레코드가 둘이라 질문 목록·좋아요 집계가 분리되고, Ballroom 1 참가자와 Ballroom 2 참가자는 서로의 질문을 못 본다. 진행자가 한쪽만 보면 **다른 룸 질문이 통째로 묻힌다.**
  - **대응(운영):** 그 슬롯에는 룸 운영자 화면 두 개(`#/ra/feb8a3/4809`, `#/ra/feb8a3/53c4`)를 모두 띄워 두도록 핸드오버 문서의 **행사 중 절차와 데이터 표에 명시**.
  - **대응(구조, 미적용):** 두 룸이 하나의 Q&A 를 공유하려면 세션을 여러 트랙에 걸 수 있는 구조가 필요하다(스키마 변경). 지금은 미지원이며, 행사까지 3주라 운영으로 흡수하는 편이 안전하다고 판단.
- **③ 공개/비공개** — 41개 중 33 공개 / 8 비공개로 담당자가 조정 완료. 문서의 "12개 비공개, 의도 확인 필요" 항목 갱신.
- **결과:** 핸드오버의 "데이터 정리가 필요한 것" 표가 **"데이터 관련"(전부 종결/담당 배정)** 으로 바뀜.

### 47) 🔑 트랙 고정 코드(`tracks.code`) — 룸 QR 이 트랙 순서에 의존하던 문제 제거
- **지시:** "해줘" (남은 숙제 ③)
- **문제:** 룸 QR 주소의 마지막 세그먼트가 `tracks.sort_order` 였다. 순서는 관리자가 언제든 바꿀 수 있어서 **인쇄물을 붙인 뒤 순서를 바꾸면 QR 이 다른 룸을 가리킨다.** 현장에서 되돌릴 수 없는 사고.
- **처리**
  - `add_track_codes.sql` 신규 — `tracks.code` 컬럼 + 4자리 hex 백필 + 유니크 인덱스 + 신규 트랙 default. **사용자가 실행 완료** (4809·53c4·782c·fd7b 부여 확인).
  - `server.js` — 룸 조회가 **코드/숫자 둘 다** 수용. `fetchTracksForProject` 는 code 컬럼 없는 환경에서 컬럼을 빼고 재시도(하위호환). `mapTrackRow`·룸 응답·`rooms[]` 에 code 포함.
  - `index.html` — `roomSeg(room)` 도입: 코드가 있으면 코드, 없으면 번호. QR 생성·URL 복사·운영자 화면 열기·다른 룸 보기 전부 이 규칙을 따른다.
- **🐞 배포 후 발견한 버그(즉시 수정):** 코드 `4809` 만 404. **4자리 hex 는 전부 숫자인 경우가 흔한데(약 15%), 숫자 판정을 먼저 해서 그 코드를 룸 번호로 해석**했다. → **코드 조회를 먼저** 하고, 매칭 실패 시에만 숫자로 해석하도록 순서 교체. 코드는 4자리라 한 자리 룸 번호와 겹치지 않는다.
- **검증 (프로덕션)**
  - 코드 주소 4개 전부 정상(`4809` 포함), 기존 숫자 주소 1~4 도 그대로 동작, `zzzz`·`9`·`0` → 404.
  - **핵심 — 순서 변경 내성:** 버리는 프로젝트(룸 A/B)에서 `sort_order` 를 서로 뒤집었더니 **코드 주소는 그대로 A/B 를 가리키고, 숫자 주소만 서로 뒤바뀌었다.** 없애려던 문제가 정확히 재현·해소됨.
  - 콘솔 룸 QR 모달이 코드 주소(`/r/feb8a3/4809` …) 생성, 참가자 '다른 룸 보기' 도 코드 주소(`#/r/feb8a3/fd7b`)로 이동. 콘솔 에러 0.
  - ⚠️ 첫 확인 때 QR 모달이 숫자 주소를 보여줬는데 **브라우저 캐시**였다(하드 리로드 후 코드 주소). CDN 캐시(s-maxage=300)를 붙인 뒤로는 배포 직후 확인 시 강제 새로고침이 필요하다.
  - 정리: 테스트 프로젝트 삭제 → questions 0 / votes 0 / post_throttle 0, 실제 트랙 순서·코드 무결.
- **배포:** `dpl_CjwsfYwxcVCjC66FFQ2EtTscFXnJ` → 버그 수정 후 `dpl_9VCkbgp6pTuQEq5BXRDEQqQ7ueba` READY.
- **문서:** 개발자 설명서 §6 ① 을 "코드 우선, 숫자는 하위호환"으로 다시 씀(+ 코드 먼저 조회해야 하는 이유 명시), SQL 순서에 10번 추가. 핸드오버는 남은 숙제 4→3건, 행사 전 절차의 "트랙 순서 고정" 경고를 "코드라서 순서 변경에 안전, 단 트랙을 지웠다 다시 만들면 재발급" 으로 교체.

### 46) ✅ 질문 도배 제한 적용 확인 (사용자가 SQL 실행 완료)
- **지시:** "해놨어" (`fix_question_rate_limit.sql` 실행 완료 통보) → 말만 믿지 않고 실제로 막히는지 끝까지 검증.
- **검증 (버리는 프로젝트 `[제한검증] 삭제예정` 생성 → 검증 → 삭제)**
  - **① 트리거가 살아났다:** anon 질문 1건 등록 → `post_throttle` **0 → 1건** 기록, `fp` 는 md5 IP 지문(`804df1b6…`, 앞선 `votes.voter_fp` 와 동일 IP). **수정 전에는 20건을 넣어도 계속 0이었다.**
  - **② 상한이 정확히 동작:** 연속 등록 → **60건 성공 / 61번째 `QUESTION_RATE_LIMITED` 차단**.
  - **③ 참가자 화면 문구:** 차단 상태에서 실제 브라우저로 질문 등록 시도 → 모달이 닫히지 않고 **"질문이 너무 빠르게 등록됐어요. 잠시 후 다시 시도해 주세요"** 표시. (일반 실패 토스트가 아니라 사유별 안내로 나가는 것까지 확인)
- **정리:** 테스트 프로젝트 삭제(세션 1·질문 60 cascade) + `post_throttle` 즉시 비움 → questions 0 / votes 0 / post_throttle 0. 실제 데이터 그대로(프로젝트 2·세션 45·트랙 4).
- **문서:** `HANDOVER.md` 의 남은 숙제에서 이 항목 제거(6건 → 4건), 체크리스트는 완료 표시하되 **"새 Supabase 프로젝트를 만들 때는 다시 실행해야 한다"**는 단서를 남김. 스트레스 테스트 절의 "아직 실행 전" 문구도 검증 결과로 교체.
...
- **🔴 최대 발견 — 함수가 미국 동부에서 돌고 있었다.** 응답 헤더 `x-vercel-id: icn1::iad1` = 서울 엣지로 들어와 **iad1(미 동부)에서 실행**. DB(Supabase)는 서울이라 **쿼리마다 태평양 왕복**. 랜딩은 순차 쿼리가 4번이라 그대로 1초가 됐다.
  - 조치: `vercel.json` 에 `"regions": ["icn1"]` 추가 후 재배포 → `icn1::icn1` 확인.
  - **결과 (p50, 동시성 10 기준): 첫 진입 572→53ms, 룸 타임테이블 947→92ms, 랜딩 1111→106ms, 세션 단건 891→83ms — 전부 약 10배.** RPS 도 랜딩 8.4→88.
  - ⚠️ `vercel.json` 에 `//regions` 같은 주석 키를 넣으면 스키마 검증에서 배포가 거부된다(경험).
- **⚡ CDN 캐시 추가:** `index.html`(246KB, gzip 62KB)은 SPA fallback 이라 **모든 첫 진입이 함수를 깨운다.** 세션 시작 시각 QR 스캔 몰림이 최대 부하 지점이라 `Cache-Control: public, max-age=0, s-maxage=300, stale-while-revalidate=86400` 부여 → 2번째 요청부터 `x-vercel-cache: HIT`. API 응답은 캐시되지 않음을 확인.
- **🐞 질문 도배 제한이 처음부터 무효였다 (부하 테스트가 아니었으면 못 찾음).** anon 으로 질문 20건을 연속 등록했는데 **전부 통과**하고 `post_throttle` 에 **기록 0건**.
  - **원인:** `questions_rate_limit()` 는 security definer 인데 그 안에서 `current_user not in ('anon','authenticated')` 로 호출자를 판별했다. **security definer 함수 안의 `current_user` 는 호출자가 아니라 함수 소유자**라서 조건이 항상 참 → 매번 즉시 `return new`.
  - **대조 근거:** 같은 파일의 `like_question()` 은 이 가드가 없어 정상 동작(61번째에서 `rate_limited`), `votes.voter_fp` 에 md5 지문도 정상 기록 → `client_fp()` 자체는 멀쩡. 금지어 트리거도 정상(`BANNED_WORD` 23514) → questions 트리거가 붙어 있는 것도 맞음. 즉 문제는 role 판별 한 줄.
  - **수정:** `fix_question_rate_limit.sql` 신규 — `current_setting('role', true)` 로 판별(PostgREST 가 요청마다 `SET LOCAL ROLE` 하고, definer 진입은 이 GUC 를 바꾸지 않는다). anon→적용, service_role→면제, SQL Editor→면제.
  - **한도 10 → 60 으로 상향.** 이 제한은 IP 지문 단위인데 행사장 공용 와이파이는 NAT 라 **룸 전체가 한 IP** 다. 60초 10건이면 관객 수백 명의 정상 질문이 막힌다(지금까지는 제한이 무효라 드러나지 않았을 뿐). 60건/분이면 스크립트 도배는 막고 사람 손 질문은 통과.
  - **⚠️ 사용자가 SQL Editor 에서 실행해야 적용된다.** 미실행 시 현 상태(=제한 없음) 유지.
- **측정 결과 요약 (리전 수정 후, 실패 0건)**
  | 시나리오 | 동시성10 p50 | 동시성100 p50 | 동시성100 p95 |
  |---|---:|---:|---:|
  | index.html 첫 진입 | 53ms | 424ms | 1125ms |
  | 룸 타임테이블 API | 92ms | 356ms | 618ms |
  | 행사 랜딩 API | 106ms | 441ms | 704ms |
  | 세션 단건 API | 83ms | 235ms | 386ms |
  | 질문 목록(Supabase 직결) | 26ms | 156ms | 323ms |
- **Realtime 동시 구독: 300개까지 실패 0**, 구독 지연 p50 ~500ms 로 평탄. 참가자 1명 = 구독 1개이므로 룸당 수백 명 수용 가능 확인.
- **쓰기 경로:** 질문 등록 p50 40ms, 좋아요 p50 29ms. 좋아요는 60표/분에서 정상 차단.
- **📏 측정 한계(정직하게):** `index.html` 처리량이 동시성과 무관하게 ~155 RPS 에서 평탄한데, 이는 서버가 아니라 **테스트 PC 회선 포화**다(155 × 62KB ≈ 9.3MB/s ≈ 77Mbps, 단일 스트림 실측과 일치). 즉 첫 진입 처리량 수치는 서버 상한이 아니라 **하한**이다. 진짜 상한을 보려면 분산 부하 도구(k6 Cloud 등)가 필요하다.
- **정리:** 테스트 프로젝트 삭제(세션 3·질문 20 cascade) → questions 0 / votes 0 / post_throttle 0, 실제 데이터 그대로(프로젝트 2·세션 45·트랙 4).
- **배포:** `dpl_3j6u47jb2pKYshyi7LyPPfva1vnM`(리전) → `dpl_HHZ6TsFxBG2aPDzo7R9PbW2YExky`(CDN 캐시) 둘 다 Production READY.

### 43) 📗 문서 2건 갱신 + griff_pj 푸시
- **지시:** "다해줘" (42번 끝의 문서 반영 + 푸시 제안 수락)
- **`DEVELOPER_GUIDE.md`** — 라우트표에 `#/ra/:projectCode/:roomNo` 추가하고, **두 룸 화면이 `useRoomAutoSwitch` 를 공유한다**는 점과 "한쪽만 고쳐서 두 화면이 다른 세션을 보게 만들지 말 것"을 명시. `#/ra/…` 가 콘솔 토큰 게이트를 쓰는 이유(세션마다 admin_token 이 달라 룸 단위 URL 토큰 불가) 설명 추가. 코드 지도 행 번호를 현재 파일 기준으로 전부 갱신(`RoomQrModal` 1465 / `useRoomAutoSwitch` 3783 / `RoomAdminPage` 3889 등).
- **`HANDOVER.md`** — 화면 종류에 **룸 운영자 화면** 추가, "되는 것"에 신규 3종 반영, **남은 숙제를 7건 → 5건으로 재정리**(②③④ 해소, 트랙 고정 코드 항목 신설). 행사 전 절차 4·5번을 "콘솔 → 룸 QR 버튼으로 PNG 발급" + "룸 PC 에서 `#/ra/…` 1회 로그인"으로 교체. **장애 대응표에 룸 관련 3행 추가**(세션 전환 안 됨 / QR 이 엉뚱한 룸 / 운영자 화면 로그인 반복). 체크리스트도 4개 화면 기준으로 수정.
- **푸시:** `dbbab11..553bd23` → `goms-blip/griff_pj` main. `LiveQnA/index.html`·`server.js`·`WORKFLOW.md` 갱신 + `DEVELOPER_GUIDE.md`(17.5KB)·`HANDOVER.md`(10.2KB) 신규.
  - 시크릿 스캔 통과. ⚠️ 스캐너가 `WORKFLOW.md` 를 한 번 잡았는데, **40번 항목에 적어둔 스캔 설명 문구(`gho_|ghp_|sk-|AIza`)가 자기 자신에 매칭된 오탐**이었음(실제 값 아님).
  - 원격 반영 확인: `RoomQrModal`·`useRoomAutoSwitch`·`RoomAdminPage`·`/ra/:projectCode` 전부 존재.
